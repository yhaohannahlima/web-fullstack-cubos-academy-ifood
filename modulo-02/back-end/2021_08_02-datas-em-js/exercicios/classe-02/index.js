...
const ordenaLista = listaPessoas.sort((a, b) => {
    const dataDeRegistroA = new Date(a.registered);
    const dataDeRegistroB = new Date(b.registered);

    if (+dataDeRegistroA < +dataDeRegistroB) {
        return -1;
    } else if (+dataDeRegistroB < +dataDeRegistroA) {
        return 1;
    } else {
        return 0;
    }
